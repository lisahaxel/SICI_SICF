...
from TMS_EEG_SICI_SICF_moabb import (SICIClassification, SICFClassification, SinglePulseClassification, SICIDataset, SICFDataset, SinglePulseDataset)
...
from utils import (RegressionMetricsTracker, evaluate_single_trial,
                   evaluate_zero_shot, filter_args_for_model,
    ...
    try:
        ...
        if dataset_name in ["TMSEEGClassification", "TMSEEGClassificationTEP", "TMSEEGClassificationTEPfree"]:
            ...
            try:
                ...
                elif dataset_name == "TMSEEGClassificationTEP":
                    dataset = TMSEEGDatasetTEP()
                    paradigm = TMSEEGClassificationTEP(tmin=args.tmin, tmax=args.tmax)
                elif dataset_name == "SICIClassification":
                    dataset = SICIDataset()
                    paradigm = SICIClassification(tmin=args.tmin, tmax=args.tmax)
                elif dataset_name == "SICFClassification":
                    dataset = SICFDataset()
                    paradigm = SICFClassification(tmin=args.tmin, tmax=args.tmax)
                else: # dataset_name == "SinglePulseClassification":
                    dataset = SinglePulseDataset()
                    paradigm = SinglePulseClassification(tmin=args.tmin, tmax=args.tmax)
