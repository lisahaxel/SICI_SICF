...
def run_subject_processing(subject_id: str):
    """
    Main logic to preprocess data for a single subject, focusing on
    pre-stimulus EEG and EMG (MEP) extraction.
    """
    # --- 1. Setup Paths and Global Parameters ---
    # NOTE: Path now points to where the epoched .fif files are expected to be
    ...
    save_results = True
    processed_path = Path(f"/mnt/lustre/work/macke/mwe626/repos/eegjepa/EDAPT_neurips/EDAPT_TMS/SICISICF/data_processed_final_pre_ica_{use_ica_on_pre}_final")

    # Time ranges (only pre-stim and EMG are needed)
    ...
    # --- 2. Define Processing Options (Post-stimulus options removed) ---
    ...
    # NOTE: Loading from a single -epo.fif file, as generated by your second script.
    # mne.read_epochs automatically handles split files (e.g., -1.fif) if they exist.
    ...
    # Use reorder_channels from your original script if channel order is critical
    ...
    while True:
        ...
        # Call the simplified calibration function
        out = calibrate_prestim_and_emg(epochs_now, emg_epochs_now, pre_range_times, emg_times,
    ...
    # Unpack the simplified output
    epochs_pre_calibration, epochs_emg_calibration, ica, channels_ptps, preprocessing_info = out
    
    # --- 5. Process Remaining Trials with Calibrated Parameters ---
    processing_times = {'pre': [], 'emg': [], 'calibration_times': calibration_processing_times, 'n_calibration_iterations': len(trials_in_calibration)}
    
    preprocessing_info.update({
        'n_trials_calibration': len(epochs_pre_calibration),
        'used_emg_channel': picked_channel,
        'n_trials_used_in_calibration': n_trials_use,
        'n_trials_original': len(epochs),
        'n_trials_to_process_with_calibrated_filters': len(epochs) - n_trials_use
    })
    ...
    for trial_ind in range(n_trials_use, len(epochs)):
        ...
        epoch_emg_single = emg_epochs[trial_ind]

        # Process Pre-stimulus EEG
        epoch_pre_now = epoch.copy().crop(pre_range[0], pre_range[1]).resample(resample_to, method='polyphase')
        epoch_pre, _ = preprocess_pre_trial_with_calibrated_data(epoch_pre_now, preprocessing_info, trial_rejection_options, ica, filter_options, ica_options, use_ica_on_pre)

        # Process EMG
        epoch_emg_single.crop(emg_times[0], emg_times[1]).resample(resample_to, method='polyphase')
        epoch_emg_processed, mep_ptp, _ = preprocess_emg_trial(epoch_emg_single, filter_options_emg, emg_trial_rejection_options, preprocessing_info, line_freq)

        # Append trial if both parts are good
        if epoch_pre and epoch_emg_processed:
            epochs_pre_list.append(epoch_pre)
            epochs_emg_list.append(epoch_emg_processed)
            ch_ptps.append(mep_ptp)
        else:
            bad_trials_calibrated.append(trial_ind)

    # --- 6. Final Aggregation and Metadata ---
    ...
    bad_keys = ['bad_trials_ocular', 'bad_trials_pre', 'bad_trials_emg']
    block_idents_calib = block_identifiers[:n_trials_use]
    for key in bad_keys:
        if key in preprocessing_info:
            current_good_trials = np.array([index for index in range(len(block_idents_calib)) if index not in preprocessing_info[key]])
            block_idents_calib = block_idents_calib[current_good_trials]

    good_trials_calibrated_indices = np.array([index for index in range(n_trials_use, len(epochs)) if index not in bad_trials_calibrated])
    block_idents_remaining = block_identifiers[good_trials_calibrated_indices]
    
    final_block_identifiers = np.concatenate((block_idents_calib, block_idents_remaining))
    ...
    if save_results:
        # Save epoch structures (only pre and emg)
    ...
    # It's good practice to explicitly delete large objects to free memory
    del epochs, emg_epochs, epochs_pre, epochs_emg, epochs_pre_list, epochs_emg_list
    del epochs_pre_calibration, epochs_emg_calibration, preprocessing_info, ica, channels_ptps
...
# %%
run_subject_processing(subject_id="SICI-SICF_sub-01")

# %%
run_subject_processing(subject_id="SICI-SICF_sub-02")
run_subject_processing(subject_id="SICI-SICF_sub-03")
run_subject_processing(subject_id="SICI-SICF_sub-04")
run_subject_processing(subject_id="SICI-SICF_sub-05")
run_subject_processing(subject_id="SICI-SICF_sub-06")
run_subject_processing(subject_id="SICI-SICF_sub-07")
run_subject_processing(subject_id="SICI-SICF_sub-08")
run_subject_processing(subject_id="SICI-SICF_sub-09")
